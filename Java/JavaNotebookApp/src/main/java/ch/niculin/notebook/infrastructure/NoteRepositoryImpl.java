...
public class NoteRepositoryImpl implements NoteRepository {
    ...
    public NoteRepositoryImpl(List<Note> notebook) {
    ...
    private Note getNoteById(NoteId id) {
        return notebook.stream().filter(note -> note.getNoteId().equals(id)).findFirst().orElseThrow(IndexOutOfBoundsException::new);
    }
}
